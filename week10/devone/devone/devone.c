...
#include <linux/uaccess.h> //update 2/23
...
static void devone_timeout(struct timer_list *arg)//update 2/23
{
...
static void devone_irq(struct timer_list *arg)//update 2/23
{
...
int devone_open(struct inode *inode, struct file *filp)
{
	...
	dev->timeout_done = 0;
	//(1)
	timer_setup(&dev->timeout, devone_timeout, 0);//update 2/23
	//init_timer(&dev->timeout);
	//dev->timeout.function = devone_timeout;
	//dev->timeout.data = (unsigned long)dev;
	
	//(2)
	timer_setup(&dev->irq, devone_irq, 0);//update 2/23
	//init_timer(&dev->irq);
