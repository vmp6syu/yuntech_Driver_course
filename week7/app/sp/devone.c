 ...
 * devone.c
 *
 * Update: 2007/12/19,2011/8/26 Yutaka Hirata
 */
...
/* driver parameter */
/* パーミッションに S_IRUGO を指定しておくと、/sys/module/<module_name/parameters/
 * で、ドライバパラメータを読み書きすることができる。
 */
module_param(timeout_value, uint, S_IRUGO | S_IWUSR);
...
int devone_open(struct inode *inode, struct file *filp)
{
	...
	/* セマフォの代わりに、性能がよいmutexが登場したため、従来の
	 * セマフォをmutex代わりと使っていたコードが削除された。
	 * init_MUTEX()もその1つで、sema_init() に置き換える。
	 */
	sema_init(&dev->sem, 1);
	//4.15------------------------------------------
